...
int main (int argc,char *argv[])					//.text
{
    ...
    int li0 = 0;                             //没有static的局部变量在寄存器或栈里
    int li1 = 1;                            //没有static的局部变量在寄存器或栈里
   ...
   //打印系统变量信息
    ...
    p1 = malloc(1L << 28);   // 256Mb，  在mmap
    p2 = malloc(1L << 17);   // 128Kb,  小于等于128Kb，在heap-brk
    p3 = malloc( (1L << 17) +1);   //128Kb + 1byte， 在mmap,共享内存高到低
    p4 = malloc(1L << 30);      // 1Gb， 在mmap
    p5 = malloc(1L << 31);      // 2Gb， 在mmap
